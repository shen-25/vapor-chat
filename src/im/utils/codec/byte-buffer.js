export default class ByteBuffer {
  ...
  uint32(val, index) {
    if (arguments.length == 0) {
      this._list.push(
        this._org_buf.getUint32(this._offset, this._littleEndian)
      );
      this._offset += 4;
    } else {
      this._list.splice(index != undefined ? index : this._list.length, 0, {
  ...
  int64(val, index) {
    if (arguments.length == 0) {
      this._list.push(
        this._org_buf.getFloat64(this._offset, this._littleEndian)
      );
      this._offset += 8;
    } else {
      this._list.splice(index != undefined ? index : this._list.length, 0, {
  ...
  float(val, index) {
    if (arguments.length == 0) {
      this._list.push(
        this._org_buf.getFloat32(this._offset, this._littleEndian)
      );
      this._offset += 4;
    } else {
      this._list.splice(index != undefined ? index : this._list.length, 0, {
  ...
  byteArray(val, len, index) {
    ...
    if (val == undefined || val == null) {
      var arr = new Uint8Array(
        this._org_buf.buffer.slice(this._offset, this._offset + len)
      );
      this._list.push(arr);
      this._offset += len;
    } else {
   ...
   */
  pack(ifHead) {
    this._org_buf = new DataView(
      new ArrayBuffer(ifHead ? this._offset + 4 : this._offset)
    );
    var offset = 0;
    if (ifHead) {
